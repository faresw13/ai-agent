...
# =========================================================
# THEME TEST - Existing Salla Theme Editor
# =========================================================

@app.route("/agent/theme-test", methods=["GET"])
def agent_theme_test():
    row = get_connected_store()
    ...
    component_id = request.args.get("component_id", "634256924")
    version_id = request.args.get("version_id", "1281089670")

    url = (
        "https://api.salla.dev/admin/v2/design/component/"
        f"{component_id}?version_id={version_id}"
    )

    try:
        result = salla_request(
            access_token,
            "GET",
            f"/design/component/{component_id}?version_id={version_id}"
        )
        ...
        return jsonify({
            ...
            "salla_response": result
        }), 200

    except Exception as e:
        print(
            f"Salla theme test error: "
            f"{type(e).__name__}: {e}"
        )
        ...
        return jsonify({
            ...
            "component_id": component_id,
            "version_id": version_id,
            "message": "Salla theme test failed",
